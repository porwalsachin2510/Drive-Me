...
const B2B_FleetVehicleAssignmentSection = ({
  ...
  // Check payment mode - EMI mode doesn't require advance payment for vehicle assignment
  ...
  const getPaymentStatus = () => {
    // For EMI Payment Mode - different flow
    if (isEMIPaymentMode) {
      if (isActive) {
        if (areAllVehiclesAssigned) {
          return {
            ...
            message:
              "All requested vehicles have been successfully assigned to the corporate contract.",
          };
          ...
          return {
            ...
            message:
              "Some vehicles have been assigned. Complete the assignment for all requested vehicles.",
          };
        } else {
          return {
            status: "ready",
            ...
            message:
              "EMI Payment Plan is active. You can now assign vehicles from your fleet.",
          };
        }
      } else {
        return {
          status: "pending",
          title: "EMI Plan Setup Required",
          message:
            "Corporate owner needs to activate EMI payment plan before vehicle assignment.",
        };
      }
    }

    // For STANDARD Payment Mode - original flow
    if (!isAdvancePaid) {
      return {
        status: "pending",
    ...
    if (!isSecurityDepositPaid && !isSecurityDepositWaived) {
      return {
        status: "partial",
        ...
        <div className="section-content">
          ...
          <div className="payment-breakdown">
            ...
            <div className="payment-items">
              {isEMIPaymentMode ? (
                /* EMI Payment Mode Display */
                <>
                  <div
                    className={`payment-item ${isActive ? "paid" : "pending"}`}
                  >
                    <span className="item-icon">{isActive ? "✓" : "⏳"}</span>
                    <div className="item-details">
                      <span className="item-name">EMI Payment Plan</span>
                      <span className="item-note">
                        {contract.financials?.emiPlanSummary?.tenure || 6}{" "}
                        months tenure
                      </span>
                      <span className="item-amount">
                        {currency}{" "}
                        {contract.financials?.emiPlanSummary?.monthlyEMI?.toFixed(
                          2,
                        ) || "0.00"}
                        /month
                      </span>
                  ...
                  <div
                    className={`payment-item ${isSecurityDepositWaived ? "paid" : isSecurityDepositPaid ? "paid" : "pending"}`}
                  >
                    <span className="item-icon">
                      {isSecurityDepositWaived
                        ? "✓"
          ...
          {isActive && areAllVehiclesAssigned && (
            <div className="assignment-completed">
              ...
              <p className="completed-message">
                All requested vehicles have been successfully assigned to this
                corporate contract.
              </p>
              <div className="assignment-summary">
                <span className="summary-text">
                  {contract.vehicles?.reduce(
                    (total, vehicle) =>
                      total + (vehicle.assignedVehicles?.length || 0),
                    0,
                  )}{" "}
                  vehicles assigned across {contract.vehicles?.length || 0}{" "}
                  vehicle types
                </span>
              </div>
            </div>
          ...
          {!isActive && (
            <div className="assignment-blocked">
              <p>
                {isEMIPaymentMode
                  ? "Vehicle assignment is blocked until the EMI payment plan is activated by the corporate owner."
                  : "Vehicle assignment is blocked until advance payment is received from the corporate owner."}
              </p>
            </div>
          )}
            ...
            <div className="vehicles-list">
              {contract.vehicles?.map((vehicle, idx) => {
                ...
                const isPartiallyAssigned =
                  assignedCount > 0 && assignedCount < vehicle.quantity;

                return (
                  <div
                    key={idx}
                    className={`vehicle-item-info ${isFullyAssigned ? "fully-assigned" : isPartiallyAssigned ? "partially-assigned" : "not-assigned"}`}
                  >
                    ...
                    <span className="assignment-status">
                      {isFullyAssigned ? (
                        <span className="status-badge completed">
                          ✓ {assignedCount}/{vehicle.quantity}
                        </span>
                      ) : isPartiallyAssigned ? (
