...
function AdminUserDetailsModal({ user, onClose }) {
  ...
  const getRoleLabel = (role) => {
    // A super admin is stored as role "ADMIN" with adminPermissions.isSuperAdmin = true.
    if (role === "ADMIN" && user?.adminPermissions?.isSuperAdmin === true) {
      return "Super Admin";
    }
    const roleLabels = {
      COMMUTER: "Commuter",
  ...
  const cashCancellation = user.cashCancellation || {};
  const hasCancellationDue =
    Number(cashCancellation.outstandingDue) > 0 || cashCancellation.isBlocked;
    ...
    <div className="admin-user-details-modal-overlay" onClick={onClose}>
      ...
      >
        ...
        <div className="admin-user-details-modal-body">
          <div className="admin-user-details-info-grid">
            <div className="admin-user-details-info-item">
              ...
              <span
                className={`admin-user-details-status admin-user-details-status-${user.status?.toLowerCase()}`}
              >
                {user.status || "N/A"}
              </span>
            </div>
            <div className="admin-user-details-info-item">
              ...
              <span className="admin-user-details-value">
                {user._id || "N/A"}
              </span>
            </div>
            <div className="admin-user-details-info-item">
          ...
          {hasCancellationDue && (
            <div className="admin-user-details-section">
              ...
              <div className="admin-user-details-route">
                <div className="admin-user-details-route-item">
                  <span className="admin-user-details-label">
                    Outstanding Due
                  </span>
                  <span className="admin-user-details-value">
                    {cashCancellation.currency || "KWD"}{" "}
                    {Number(cashCancellation.outstandingDue || 0).toFixed(2)}
                  </span>
                </div>
                <div className="admin-user-details-route-item">
                  <span className="admin-user-details-label">
                  ...
                  <span className="admin-user-details-value">
                    {cashCancellation.strikeCount || 0}
                  </span>
                </div>
                <div className="admin-user-details-route-item">
                  <span className="admin-user-details-label">
                  ...
                  <span className="admin-user-details-value">
                    {cashCancellation.isBlocked ? "Yes" : "No"}
                  </span>
                </div>
                {cashCancellation.blockedReason && (
                  <div className="admin-user-details-route-item">
                    <span className="admin-user-details-label">Reason</span>
                    <span className="admin-user-details-value">
                      {cashCancellation.blockedReason}
                    </span>
                  </div>
        ...
        <div className="admin-user-details-modal-footer">
          <button
            className="admin-user-details-btn admin-user-details-btn-secondary"
            onClick={onClose}
          >
            Close
          </button>
